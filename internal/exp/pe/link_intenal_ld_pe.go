...
func (sect *peSection) checkOffset(off int64) {
	if off != int64(sect.pointerToRawData) {
		panic(fmt.Errorf("%s.PointerToRawData = %#x, want %#x", sect.name, uint64(int64(sect.pointerToRawData)), uint64(off)))
	}
}
...
func (sect *peSection) checkSegment(seg *sym.Segment) {
	if seg.Vaddr-uint64(PEBASE) != uint64(sect.virtualAddress) {
		panic(fmt.Errorf("%s.VirtualAddress = %#x, want %#x", sect.name, uint64(int64(sect.virtualAddress)), uint64(int64(seg.Vaddr-uint64(PEBASE)))))
	}
	if seg.Fileoff != uint64(sect.pointerToRawData) {
		panic(fmt.Errorf("%s.PointerToRawData = %#x, want %#x", sect.name, uint64(int64(sect.pointerToRawData)), uint64(int64(seg.Fileoff))))
	}
}
...
func (f *peFile) addDWARFSection(name string, size int) *peSection {
	if size == 0 {
		panic(fmt.Errorf("DWARF section %q is empty", name))
	}
