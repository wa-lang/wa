...
func (s *Segment) Data() ([]byte, error) {
	return saferio_ReadDataAt(s.sr, s.Filesz, 0)
}
...
func (s *Section) Data() ([]byte, error) {
	return saferio_ReadDataAt(s.sr, s.Size, 0)
}
...
func NewFile(r io.ReaderAt) (*File, error) {
	...
	dat, err := saferio_ReadDataAt(r, uint64(f.Cmdsz), offset)
	if err != nil {
		return nil, err
	}
	c := saferio_SliceCap_Load(uint64(f.Ncmd))
	if c < 0 {
	...
	for i := uint32(0); i < f.Ncmd; i++ {
		...
		case LoadCmdSymtab:
			...
			strtab, err := saferio_ReadDataAt(r, uint64(hdr.Strsize), int64(hdr.Stroff))
			if err != nil {
				return nil, err
			}
			var symsz int
			...
			symdat, err := saferio_ReadDataAt(r, uint64(hdr.Nsyms)*uint64(symsz), int64(hdr.Symoff))
			if err != nil {
				return nil, err
			}
		...
		case LoadCmdDysymtab:
			...
			dat, err := saferio_ReadDataAt(r, uint64(hdr.Nindirectsyms)*4, int64(hdr.Indirectsymoff))
			if err != nil {
				return nil, err
			}
...
func (f *File) parseSymtab(symdat, strtab, cmddat []byte, hdr *SymtabCmd, offset int64) (*Symtab, error) {
	bo := f.ByteOrder
	c := saferio_SliceCap_Symbol(uint64(hdr.Nsyms))
	if c < 0 {
...
func (f *File) pushSection(sh *Section, r io.ReaderAt) error {
	...
	if sh.Nreloc > 0 {
		reldat, err := saferio_ReadDataAt(r, uint64(sh.Nreloc)*8, int64(sh.Reloff))
		if err != nil {
			return err
		}
