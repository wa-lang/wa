...
const (
	Helf     HeadType = iota + 1 // 未知平台的 elf 格式
	Hdarwin                      // darwin 平台的 MachO格式
	Hlinux                       // linux 平台的 elf 格式
	Hwindows                     // windows 平台的 pe 格式
)
...
const ABase = 100 // 跨平台的汇编指令的开始编号, 前的空间保留给普通的 token.Token

const (
	ABaseX86 = (1 + iota) << 11 // 不同平台的指令开始编号, 开始一段空间需要给通用汇编指令预留, 便于构造汇编指令表
	ABaseARM
	ABaseAMD64
	ABaseARM64
	ABaseLoong64
	...
	AllowedOpCodes = 1 << 11            // 每个平台最多的指令数
	AMask          = AllowedOpCodes - 1 // 不同平台的指令区间段不得超出这个大小, 可用于对应查询表格
)
...
const (
	...
	RBase    RBaseType = ABaseMax // 从汇编指令后面的空闲空间给寄存器分配编号
)

const (
	RBase386 RBaseType = RBase + iota*1024
	RBaseAMD64
	RBaseARM
	RBaseARM64
	RBaseRISCV
...
const (
	...
	ACALL          As = As(ABase) + iota // 调用函数(ABase之前是给普通Token预留的空间)
