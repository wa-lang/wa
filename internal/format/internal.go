...
const (
	srcType_File sourceType = iota
	srcType_Expr
)

// parse parses src, which was read from the named file,
// as a Go source file, declaration, or statement list.
func parse(fset *token.FileSet, filename string, src []byte, fragmentOk bool, srcType sourceType) (
	file *ast.File,
	...
	file, err = parser.ParseFile(nil, fset, filename, src, parserMode)
	// If there's no error, return. If the error is that the source file didn't begin with a
	// package line and source fragments are ok, fall through to
	// try as a source fragment. Stop and return on any other error.
	if err == nil || !fragmentOk || !strings.Contains(err.Error(), "expected 'package'") {
		// Wa 中 package 是可选的
		// Go 中没有 package 表示表达式
		// 因此通过额外的类型标注 Wa 中文件和表达式
		if srcType == srcType_File {
			return
		}
	}
	...
	file, err = parser.ParseFile(nil, fset, filename, psrc, parserMode)
	if err == nil {
		sourceAdj = func(src []byte, indent int) []byte {
