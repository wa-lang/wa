...
func (p *wat2rvWorker) Tracef(foramt string, a ...interface{}) {
	if p.trace {
		fmt.Printf(foramt, a...)
	}
}

func (p *wat2rvWorker) findGlobalName(ident string) string {
	if ident == "" {
...
func (p *wat2rvWorker) findGlobalType(ident string) token.Token {
	if ident == "" {
...
func (p *wat2rvWorker) findLocalOffset(fnNative *nativeast.Func, fn *ast.Func, ident string) int {
	if ident == "" {
		panic("wat2la: empty local name")
	}
...
func (p *wat2rvWorker) findLocalType(fn *ast.Func, ident string) token.Token {
	if ident == "" {
		panic("wat2la: empty local name")
	}
...
func (p *wat2rvWorker) findType(ident string) *ast.FuncType {
	if ident == "" {
...
func (p *wat2rvWorker) findFuncType(ident string) *ast.FuncType {
	if ident == "" {
		panic("wat2la: empty ident")
	}
...
func (p *wat2rvWorker) findFuncIndex(ident string) int {
	if ident == "" {
		panic("wat2la: empty ident")
	}
...
func (p *wat2rvWorker) makeLabelId(prefix, name, suffix string) string {
