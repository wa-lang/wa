...
//go:embed assets/native-env-linux-arm64.wa.s
var native_env_linux_arm64_wa_s string

//go:embed assets/native-env-linux-arm64.wz.s
var native_env_linux_arm64_wz_s string

// 生成运行时函数
func (p *wat2arm64Worker) buildRuntimeHead(w io.Writer) error {
	var list = []string{
		kRuntimeWrite,
		kRuntimeExit,
		kRuntimeMalloc,
...
// 生成运行时函数
func (p *wat2arm64Worker) buildRuntimeImpl(w io.Writer) error {
	if p.isZhLang {
		fmt.Fprintln(w, native_env_linux_arm64_wz_s)
	} else {
		fmt.Fprintln(w, native_env_linux_arm64_wa_s)
	}
...
func (p *wat2arm64Worker) buildRuntimeImpl_panic(w io.Writer) error {
