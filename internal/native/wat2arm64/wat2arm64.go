...
// Wat程序转译到 龙芯汇编
func Wat2ARM64(filename string, source []byte) (m *watast.Module, code []byte, err error) {
...
func newWat2LAWorker(filename string, mWat *watast.Module) *wat2arm64Worker {
	p := &wat2arm64Worker{
		m:        mWat,
...
func (p *wat2arm64Worker) BuildProgram() (code []byte, err error) {
...
func (p *wat2arm64Worker) genNextId() string {
	nextId := p.nextId
	p.nextId++
