...
func (p *wat2arm64Worker) Tracef(foramt string, a ...interface{}) {
	if p.trace {
		fmt.Printf(foramt, a...)
	}
}

func (p *wat2arm64Worker) findGlobalName(ident string) string {
	if ident == "" {
...
func (p *wat2arm64Worker) findGlobalType(ident string) token.Token {
	if ident == "" {
...
func (p *wat2arm64Worker) findLocalOffset(fnNative *nativeast.Func, fn *ast.Func, ident string) int {
	if ident == "" {
		panic("wat2la: empty local name")
	}
...
func (p *wat2arm64Worker) findLocalType(fn *ast.Func, ident string) token.Token {
	if ident == "" {
		panic("wat2la: empty local name")
	}
...
func (p *wat2arm64Worker) findType(ident string) *ast.FuncType {
	if ident == "" {
...
func (p *wat2arm64Worker) findFuncType(ident string) *ast.FuncType {
	if ident == "" {
		panic("wat2la: empty ident")
	}
...
func (p *wat2arm64Worker) findFuncIndex(ident string) int {
	if ident == "" {
		panic("wat2la: empty ident")
	}
...
func (p *wat2arm64Worker) makeLabelId(prefix, name, suffix string) string {
