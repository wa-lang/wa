...
func format(src []byte, mode checkMode) ([]byte, error) {
	// parse src
	f, err := parser.ParseFile(nil, fset, "", src, parser.ParseComments)
	if err != nil {
	...
	res := buf.Bytes()
	if _, err := parser.ParseFile(nil, fset, "", res, 0); err != nil {
...
func testComment(t *testing.T, f *ast.File, srclen int, comment *ast.Comment) {
	...
	for offs := 0; offs <= srclen; offs++ {
		...
		if _, err := parser.ParseFile(nil, fset, "", buf.Bytes(), 0); err != nil {
...
`
	...
	// parse original
	f1, err := parser.ParseFile(nil, fset, "src", src, parser.ParseComments)
	if err != nil {
		t.Fatal(err)
	}
	...
	f2, err := parser.ParseFile(nil, fset, "", buf.Bytes(), 0)
	if err != nil {
		t.Fatalf("%s\n%s", err, buf.Bytes())
	}
...
`
	...
	// parse original
	f1, err := parser.ParseFile(nil, fset, "src.go", orig, 0)
	if err != nil {
		t.Fatal(err)
	}
...
func TestDeclLists(t *testing.T) {
	for _, src := range decls {
		file, err := parser.ParseFile(nil, fset, "", "package p;"+src, parser.ParseComments)
		if err != nil {
			panic(err) // error in test
		}
...
func TestBaseIndent(t *testing.T) {
	...
	// this test fail.
	const filename = "printer.go.wa"
	src, err := ioutil.ReadFile(filename)
	if err != nil {
		panic(err) // error in test
	}

	file, err := parser.ParseFile(nil, fset, filename, src, 0)
	if err != nil {
		panic(err) // error in test
	}
...
`
	...
	fset := token.NewFileSet()
	f, err := parser.ParseFile(nil, fset, "input.go", input, parser.ParseComments)
	if err != nil {
		t.Fatal(err)
	}
...
func TestIssue11151(t *testing.T) {
	...
	_, err = parser.ParseFile(nil, fset, "", got, 0)
	if err != nil {
		t.Errorf("%v\norig: %q\ngot : %q", err, src, got)
	}
}
