...
type File struct {
	...
	capacity int    // 最大容量, 用于 file 更新预留
...
func (f *File) SetLinesForContent(content []byte) {
	if f.capacity < len(content) {
		panic("file content large than capacity")
	}
	var lines []int
	line := 0
...
func (s *FileSet) AddFile(filename string, base, size int) *File {
	return s.addFile(filename, base, size, 0)
}

// 多分配一些容量, 用于后续更新
func (s *FileSet) AddFileWithCap(filename string, base, size, cap int) *File {
	return s.addFile(filename, base, size, cap)
}

func (s *FileSet) addFile(filename string, base, size, cap int) *File {
	s.mutex.Lock()
	...
	// base >= s.base && size >= 0
	f := &File{set: s, name: filename, base: base, size: size, capacity: cap, lines: []int{0}}
	base += cap + 1 // +1 because EOF also has a position
	if base < 0 {
