...
//
func ParseFile(vfs fs.FS, fset *watoken.FileSet, filename string, src interface{}, mode Mode) (f *ast.File, err error) {
	if fset == nil {
...
//
func ParseDir(vfs fs.FS, fset *watoken.FileSet, path string, filter func(os.FileInfo) bool, mode Mode) (pkgs map[string]*ast.Package, first error) {
	fd, err := os.Open(path)
	if err != nil {
		return nil, err
	}
...
// be nil.
//
func ParseExprFrom(fset *watoken.FileSet, filename string, src interface{}, mode Mode) (ast.Expr, error) {
	if fset == nil {
...
func ParseExpr(x string) (ast.Expr, error) {
	return ParseExprFrom(watoken.NewFileSet(), "", []byte(x), 0)
}
