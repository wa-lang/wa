...
// the offending token.
//
type ErrorHandler func(pos watoken.Position, msg string)
...
type Scanner struct {
	...
	// immutable state
	file *watoken.File // source file handle
...
// of the file.
//
func (s *Scanner) Init(file *watoken.File, src []byte, err ErrorHandler, mode Mode) {
...
func (s *Scanner) scanIdentifier() string {
	...
	for isLetter(s.ch) || isDigit(s.ch) {
		// 单独处理“则”和“否则”
		if token.IsSplittingKeyword(string(s.ch)) {
			if offs == s.offset { // 前面没有任何字符，当前的词就是“则”
				s.next()
				return string(s.src[offs:s.offset])
			} else { // 前面有字符
				if string(s.src[offs:s.offset]) != "否" { // 如果前面有个“否”，需要把"否则"两个字合并起来
					break
				}
			}
		}
		s.next()
		// 如果是“若”之类的关键字，则需要在后面分词。
		if token.IsClosingKeyword(string(s.src[offs:s.offset])) {
			break
		}
	}
	return string(s.src[offs:s.offset])
...
//
func (s *Scanner) Scan() (pos watoken.Pos, tok token.Token, lit string) {
scanAgain:
	s.skipWhitespace()
	...
	case isLetter(ch):
		...
		if len(lit) > 1 {
			...
			switch tok {
			case token.IDENT, token.BREAK, token.CONTINUE, token.RETURN, token.TK_归于:
				insertSemi = true
			}
		} else {
			insertSemi = true
