...
// fn arduino.digitalRead(pin: u32) => u32
m3ApiRawFunction(m3_arduino_digitalRead) {
	m3ApiReturnType(uint32_t);
...
// fn arduino.analogRead(pin: u32) => i32
...
// fn arduino.analogWrite(pin: u32, val: i32)
m3ApiRawFunction(m3_arduino_analogWrite) {
	m3ApiGetArg(uint32_t, pin);
...
M3Result LinkArduino(IM3Runtime rt) {
	...
	m3_LinkRawFunction (rt->modules, "arduino", "digitalRead",       "i(i)",  &m3_arduino_digitalRead);
	m3_LinkRawFunction (rt->modules, "arduino", "digitalWrite",      "v(ii)", &m3_arduino_digitalWrite);
	m3_LinkRawFunction (rt->modules, "arduino", "analogRead",        "i(i)",  &m3_arduino_analogRead);
	m3_LinkRawFunction (rt->modules, "arduino", "analogWrite",       "v(ii)", &m3_arduino_analogWrite);
...
void wasm_task(void* ctx) {
	...
	Serial.println("Running Wa/WebAssembly, build " APP_WASM_BUILD_TIME);
