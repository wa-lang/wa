...
func newTypeUseParser(enabledFeatures api.CoreFeatures, module *wasm.Module, typeNamespace *indexNamespace) *typeUseParser {
...
func (p *typeUseParser) beginResult(tok tokenType, tokenBytes []byte, line, col uint32) (tokenParser, error) {
	...
	case "result":
		...
		if err := p.enabledFeatures.RequireEnabled(api.CoreFeatureMultiValue); err != nil {
