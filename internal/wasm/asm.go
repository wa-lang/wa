...
func writeSecHeader(ctxt *ld_Link, id uint8) int64 {
...
func writeSecSize(ctxt *ld_Link, sizeOffset int64) {
...
func writeBuildID(ctxt *ld_Link, buildid []byte) {
	sizeOffset := writeSecHeader(ctxt, sectionCustom)
	writeName(ctxt.Out, "wa.buildid")
...
func writeTypeSec(ctxt *ld_Link, types []*wasmFuncType) {
...
func writeImportSec(ctxt *ld_Link, hostImports []*wasmFunc) {
...
func writeFunctionSec(ctxt *ld_Link, fns []*wasmFunc) {
...
func writeTableSec(ctxt *ld_Link, fns []*wasmFunc) {
...
func writeNameSec(ctxt *ld_Link, firstFnIndex int, fns []*wasmFunc) {
	sizeOffset := writeSecHeader(ctxt, sectionCustom)
...
func writeMemorySec(ctxt *ld_Link, ldr *Loader) {
...
func writeGlobalSec(ctxt *ld_Link) {
...
func writeExportSec(ctxt *ld_Link, ldr *Loader, lenHostImports int) {
...
func writeElementSec(ctxt *ld_Link, numImports, numFns uint64) {
...
func writeCodeSec(ctxt *ld_Link, fns []*wasmFunc) {
...
func writeDataSec(ctxt *ld_Link) {
...
func writeProducerSec(ctxt *ld_Link) {
	sizeOffset := writeSecHeader(ctxt, sectionCustom)
	...
	writeName(ctxt.Out, "Wa")       // value: name
	writeName(ctxt.Out, "version")  // value: version

	writeName(ctxt.Out, "processed-by")   // field name
	writeUleb128(ctxt.Out, 1)             // number of values
	writeName(ctxt.Out, "Wa cmd/compile") // value: name
	writeName(ctxt.Out, "version")        // value: version
...
func asmb2(ctxt *ld_Link, ldr *Loader) {
	types := []*wasmFuncType{
	...
	hostImportMap := make(map[Sym]int64)
	_ = hostImportMap
	...
	//var buildid []byte
	fns := make([]*wasmFunc, 0) //len(ctxt.Textp))
	/*
...
func asmb(ctxt *ld_Link, ldr *Loader) {
	sections := []*Section{
	...
	for i, sect := range sections {
		data := ld_DatblkBytes(ctxt, int64(sect.Vaddr), int64(sect.Length))
