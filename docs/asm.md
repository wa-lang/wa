...
注: 该设计将被废弃, 新的设计请参考 nasm-gas.md 和 nasm-zh.md.
...
- 遵循本地的 ABI 规范
- 参数/返回值 根据 ABI 可能对应到 寄存器或栈内存
  - 如果是寄存器, 则对应寄存器的编号索引(区分整数寄存器和浮点数寄存器)
  - 如果是栈内存, 则对应从 FP 栈帧寄存器开始的偏移栈空间
- 局部变量, 是相对于 FP 栈帧寄存器开始的偏移栈空间
  - 为了减少栈内存碎片, 局部变量会重新排序
- 当前函数栈帧大小的常量`%func.frame_size`
- 当前函数的指令大小常量`%func.size`

参数/返回值/局部变量名字只是为了自动映射对应的寄存器/栈帧 FP 的偏移量, 具体是寄存器还是栈需要用户根据 ABI 确定.
