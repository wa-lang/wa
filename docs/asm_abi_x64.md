...
Windows-X64 内部实现细节(有些并非ABI规范):

|                 | 调用方               | 被调用方(寄存器)        | 被调用方(栈内存)       | 备注
| --------------- | -------------------- | ---------------------- | --------------------- | -----
| 参数(走寄存器)   | 写: `x.Reg`          | 读写: `x.Reg`          | 备份: `[rbp+x.RBPOff]` | 调用方分配影子空间
| 参数(走栈内存)   | 写: `[rsp+x.RSPOff]` | 读写: 无固定寄存器      | 备份: `[rbp+x.RBPOff]` | 调用方分配, rsp和rbp 分别定位
| 返回值(走寄存器) | 读: `x.Reg` 等       | 写: `x.Reg`            | 备份: `[rbp+x.RBPOff]` | 被调用方分配
| 返回值(走栈内存) | 读: `[rax+x.RETOff]` | 写: 无固定寄存器        | 备份: `[rcx+x.RETOff]` | 占用函数第一个参数(传入地址), 调用方分配
| 局部变量         | 无                   | 读写: 无固定寄存器      | 读写: `[rbp+x.RBPOff]` | 被调用方分配, 和调用方无关

注: `x.RBPOff` 可能是正数(调用方分配的空间), 也可能是负数(被调用方分配).
...
|  rbp   | 调用函数栈帧备份, 被调用函数入栈, 此时 rsp 完成 16 字节对齐
+--------+
|  arg0  | 影子空间, 由被调用函数自行准备(可选)
|  arg1  | 影子空间参数顺序调用函数自行决定
|  ....  | 影子空间大小需动态计算
|  arg?  |
+--------+
|  ret0  | 影子空间, 由被调用函数自行准备(可选)
|  ....  | 通过寄存器返回时, 对应返回值变量
+--------+<--- rbp
| local0 | 局部变量 基于 rbp 定位, 相对位置固定
| local1 |
...
Linux-X64 内部实现细节(有些并非ABI规范):

|                 | 调用方               | 被调用方(寄存器)        | 被调用方(栈内存)       | 备注
| --------------- | -------------------- | ---------------------- | --------------------- | -----
| 参数(走寄存器)   | 写: `x.Reg`          | 读写: `x.Reg`          | 备份: `[rbp+x.RBPOff]` | 被调用方分配
| 参数(走栈内存)   | 写: `[rsp+x.RSPOff]` | 读写: 无固定寄存器      | 备份: `[rbp+x.RBPOff]` | 调用方分配, rsp和rbp 分别定位
| 返回值(走寄存器) | 读: `x.Reg` 等       | 写: `x.Reg`            | 备份: `[rbp+x.RBPOff]` | 被调用方分配
| 返回值(走栈内存) | 读: `[rax+x.RETOff]` | 写: 无固定寄存器        | 备份: `[rdi+x.RETOff]` | 占用函数第一个参数(传入地址), 调用方分配
| 局部变量         | 无                   | 读写: 无固定寄存器      | 读写: `[rbp+x.RBPOff]` | 被调用方分配, 和调用方无关
