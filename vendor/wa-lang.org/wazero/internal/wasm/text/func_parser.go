...
func newFuncParser(enabledFeatures api.CoreFeatures, typeUseParser *typeUseParser, funcNamespace *indexNamespace, onFunc onFunc) *funcParser {
...
func (p *funcParser) beginInstruction(tokenBytes []byte) (next tokenParser, err error) {
	...
	if opCode >= wasm.OpcodeI32Extend8S && opCode <= wasm.OpcodeI64Extend32S {
		if err = p.enabledFeatures.RequireEnabled(api.CoreFeatureSignExtensionOps); err != nil {
	...
	if opCode == wasm.OpcodeMiscPrefix {
		if err = p.enabledFeatures.RequireEnabled(api.CoreFeatureNonTrappingFloatToIntConversion); err != nil {
			return nil, fmt.Errorf("%s invalid as %v", tokenBytes, err)
		}
		return p.beginFieldOrInstruction, nil
	}
